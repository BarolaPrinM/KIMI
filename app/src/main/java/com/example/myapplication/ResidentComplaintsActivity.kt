...
class ResidentComplaintsActivity : AppCompatActivity() {
    ...
    private fun showNewComplaintDialog() {
        ...
        val categories = arrayOf("Select Category", "Uncollected Garbage", "Spilled Waste", "Driver Behavior", "Schedule Issue", "Other")
            ...
            .enqueue(object : Callback<ApiResponse> {
                override fun onResponse(call: Call<ApiResponse>, response: Response<ApiResponse>) {
                    if (response.isSuccessful && response.body()?.success == true) {
                        CustomNotification.showTopNotification(this@ResidentComplaintsActivity, "Successful Submit Complaint", false)
                        dialog.dismiss()
                        fetchMyComplaints()
                    } else {
