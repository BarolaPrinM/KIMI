...
class UserManagementActivity : AppCompatActivity() {
    ...
    private fun filterList(query: String) {
        val sourceList = when (currentTab) {
            0 -> allResidents.filter { it.status == "approved" }
            1 -> allDrivers.filter { it.status == "approved" }
            2 -> allRequests
            3 -> (allResidents + allDrivers).sortedByDescending { it.username } // Combined records
            else -> allResidents
        }
    ...
    private fun updateList(tabIndex: Int) {
        val listToShow = when (tabIndex) {
            ...
            1 -> allDrivers.filter { it.status == "approved" }
            2 -> allRequests
            3 -> (allResidents + allDrivers)
            else -> allResidents.filter { it.status == "approved" }
        }
    ...
    private fun displayUsers(users: List<UserData>) {
        ...
        if (users.isEmpty()) {
            ...
            for (user in users) {
                ...
                val tvStatus = view.findViewById<TextView>(R.id.tv_status_badge)

                if (currentTab == 2) { // Requests Tab
                    ...
                    if (currentTab == 3) { // Records Tab
                        tvStatus.visibility = View.VISIBLE
                        tvStatus.text = user.status?.uppercase() ?: "UNKNOWN"
                        if (user.status == "approved") {
                            tvStatus.setBackgroundResource(R.drawable.driver_icon_bg) // Green-ish
                        } else {
                            tvStatus.setBackgroundResource(R.drawable.resident_icon_bg) // Red-ish
                        }
                    } else {
                        tvStatus.visibility = View.GONE
                    }
    ...
    private fun declineRequest(user: UserData) {
        ...
        pbLoading.visibility = View.VISIBLE
        RetrofitClient.instance.declineUser(registerRequest).enqueue(object : Callback<ApiResponse> {
            override fun onResponse(call: Call<ApiResponse>, response: Response<ApiResponse>) {
                if (response.isSuccessful && response.body()?.success == true) {
                    user.requestId?.let {
                        database.child("registration_requests").child(it).removeValue()
                    }
                    Toast.makeText(this@UserManagementActivity, "Request Declined and Recorded", Toast.LENGTH_SHORT).show()
                    fetchUsers() // Refresh list
                } else {
                    pbLoading.visibility = View.GONE
                    val errorMsg = response.body()?.message ?: "Decline failed"
                    Toast.makeText(this@UserManagementActivity, errorMsg, Toast.LENGTH_SHORT).show()
                }
            }
