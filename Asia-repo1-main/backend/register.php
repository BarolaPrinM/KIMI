...
if (!empty($data->username) && !empty($data->password) && !empty($data->role)) {
    try {
        ...
        if ($data->role === 'resident') {
            ...
            $status = !empty($data->status) ? $data->status : "approved";

            $stmt->execute([$data->username, $name, $hashed_password, $email, $phone, $purok, $address, $status]);
        ...
        // but we keep this here just in case of future direct admin creation.
            ...
            $stmt->execute([$data->username, $name, $email, $hashed_password, $phone, $license, $truck, $data->role, $status]);
